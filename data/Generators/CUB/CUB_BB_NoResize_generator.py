    ...
    def load_data(self, mode='train'):
        ...
        if self.bounding_box_crop:
            ...
            for line in content:
                ...
                bbox_content[id2img[line[0]].replace('\\\\','\\')] = (x, y, x+width, y+height)
        #print(bbox_content)
...
def write_to_pickle(mode, data):
    with open(os.path.join('../../', f'birds-cache-{mode}-bb-noresize.pkl'), 'wb') as f:
        pickle.dump(data, f)
...
if __name__ == '__main__':
    ...
    nbr_examples = {"train": 8204, "test": 1770, "validation": 1771}


    for mode in ['train','test','validation']:

        cub = CUBirds200('./',mode=mode,download=False, bounding_box_crop=True)
