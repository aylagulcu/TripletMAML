...
class TripletFlowers(Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, download=True , mode = 'all'):
        ...
        for mode in ['train', 'test', 'validation']:
            pickle_file = os.path.join(self.root, 'flowers-cache-' + mode + '.pkl')
            
            if not self._check_exists(mode) and download:
                print('Downloading VGG Flowers --', mode)
    ...
    def sample(self, type,k_shots = 1):
        if k_shots == 1 :
            ...
            y_support.append([0 for i in range(20)])
            y_support.append([x for i in range(5) for x in range(1,5)])
            ...
            y_query.append([0 for i in range(20)])
            y_query.append([x for i in range(5) for x in range(1,5)])
    ...
    def _check_exists(self, mode):
        return os.path.exists(os.path.join(self.root, 'flowers-cache-' + mode + '.pkl'))
...
if __name__ == "__main__":
    ...
    vgg = TripletFlowers('./',transform=transform)
    print(len(vgg.data['train']['image_data']))
    print(len(vgg.data['test']['image_data']))
    print(len(vgg.data['validation']['image_data']))

    """ print(cub.data['validation'])
    print(cub.data['validation']['image_data'])
    print(cub.data['validation']['class_dict']) """
    a = vgg.sample('train',5)
