...
def main(
    ...
    # Create model
    model = TripletCNN4(output_size= ways, hidden_size=number_of_filters, layers=Triplet_Model_Parameter[selected_model]["layers"], channels=Triplet_Model_Parameter[selected_model]["channels"], max_pool=Triplet_Model_Parameter[selected_model]["max_pool"], embedding_size=Triplet_Model_Parameter[selected_model]["embedding_size"] * (number_of_filters // 32))
    
    model.to(device)
    maml = l2l.algorithms.MAML(model, lr=fast_lr, first_order=True)
    optimizers = {'Adam': optim.Adam, 'SGD': optim.SGD}
    opt = optimizers[optimizer](maml.parameters(), lr=meta_lr) # meta-update
...
if __name__ == '__main__':
    ...
    #MINIIMAGENET
    #OMNIGLOT
    main(shots=5, selected_model="MINIIMAGENET", **{'number_of_filters': 128, 'meta_batch_size': 16, 'fast_lr': 0.01, 'adaptation_steps': 5, 'test_adaptation_steps': 10, 'meta_lr': 0.001, 'optimizer': 'Adam'})
