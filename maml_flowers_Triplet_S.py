...
def main(
    ...
    triplet_vgg_dataset = TripletVGG(root='./', download=True,
    ...
    for iteration in range(num_iterations):
        ...
        for task in range(meta_batch_size):
            learner = maml.clone()
            batch = triplet_vgg_dataset.sample("train") 
            ...
            learner = maml.clone()
            batch = triplet_vgg_dataset.sample("validation")
            evaluation_error, evaluation_accuracy = fast_adapt(batch,
                                                               learner,
                                                               combined_loss_fn,
                                                               adaptation_steps,
    ...
    torch.save(model,"./maml_model_vggsave.pt")

    # Create model using saved parameters:
    model = TripletCNN4(output_size= ways, hidden_size=64, layers=4, channels=3, max_pool=True, embedding_size=1600)
    model = torch.load("./maml_model_vggsave.pt")
    model.to(device)
    maml = l2l.algorithms.MAML(model, lr=fast_lr, first_order=True)
    ...
    for i in range(num_test_episodes):
        ...
        learner = maml.clone()
        batch = triplet_vgg_dataset.sample("test") 
        evaluation_error, evaluation_accuracy = fast_adapt(batch,
                                                           learner,
                                                           combined_loss_fn,
