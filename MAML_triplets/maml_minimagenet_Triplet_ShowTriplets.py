...
def main(
    ...
    #         learner = maml.clone()
    #         batch = triplet_imagenet_dataset.sample("train") 
    ...
    #         learner = maml.clone()
    #         batch = triplet_imagenet_dataset.sample("validation")
    #         evaluation_error, evaluation_accuracy = fast_adapt(batch,
    #                                                            learner,
    #                                                            combined_loss_fn,
    #                                                            adaptation_steps,
    ...
    # torch.save(model,"./maml_model_miniimagenet_save.pt")

    # Create model using saved parameters:
    model = TripletCNN4(output_size= ways, hidden_size=64, layers=4, channels=3, max_pool=True, embedding_size=1600)
    model = torch.load("./maml_model_miniimagenet_save.pt")
    model.to(device)
    maml = l2l.algorithms.MAML(model, lr=fast_lr, first_order=True)
